...
def softmax_smooth_ce_loss(logits, labels):
  ...
  """
  # label smoothing(Inception V2): give model less confident
  labels = tf.cast(labels, tf.int32)
  ohe = tf.one_hot(labels, 200, dtype=tf.int32)
  ce_loss = tf.losses.softmax_cross_entropy(ohe,logits,label_smoothing=0.1)
  tf.summary.scalar('loss', ce_loss)
...
def svm_loss(logits, labels):
  ...
  c = 1.0
  labels = tf.cast(labels, tf.int32)
  ohe = tf.one_hot(labels, 200, dtype=tf.float32, on_value=-200.0, off_value=1.0) # ??? on_value
