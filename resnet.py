...
def building_block(inputs, training, filters, name, kernel_size, first_layer_strides = 1):
...
def resnet_11(training_batch, config, training):
    img = tf.cast(training_batch, tf.float32)
    inputs = (img - 128.0)/128.0
    ...
    inputs = tf.layers.conv2d(inputs, filters = 64, kernel_size = 7, name = 'conv1', padding='same') # 64 7X7
    inputs = batch_norm(inputs, training, name = 'conv1')
    inputs = tf.nn.relu(inputs, name = 'conv1')
    print('conv1 inputs shape', inputs.shape)
    # (N, 64, 56, 56)
    
    inputs = building_block(inputs, training, filters = 64, name = 'conv2', kernel_size = 3, first_layer_strides = 1)
    print('conv2 inputs shape', inputs.shape)
    # (N, 64, 56, 56)
    
    inputs = building_block(inputs, training, filters = 128, name = 'conv3', kernel_size = 3, first_layer_strides = 2)
    print('conv3 inputs shape', inputs.shape)
    # (N, 128, 28, 28)
    
    inputs = building_block(inputs, training, filters = 256, name = 'conv4', kernel_size = 3, first_layer_strides = 2)
    print('conv4 inputs shape', inputs.shape)
    # (N, 256, 14, 14)
    
    inputs = building_block(inputs, training, filters = 512, name = 'conv5', kernel_size = 3, first_layer_strides = 2)
