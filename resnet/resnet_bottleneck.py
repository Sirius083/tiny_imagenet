...
将resnet_10中每个building block改成bottleneck的形式
'''
import tensorflow as tf
import numpy as np

# REG=1.0 # 由于在组合函数中有add_n * config.reg，所以这里不用再次定义系数
...
def building_block(inputs, training, filters, name, kernel_size, first_layer_strides = 1,data_format='channels_first'):
    ...
    # bottleneck 1
    stddev = np.sqrt(2 / int(inputs.shape[3]))
    inputs = tf.layers.conv2d(inputs, filters=filters/4, kernel_size = 1 ,padding='same', 
    ...
    inputs = tf.layers.conv2d(inputs, filters=filters/4, kernel_size=kernel_size,padding='same', 
                             kernel_initializer=tf.random_normal_initializer(stddev=stddev),
                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1.0),
    ...
    # bottleneck 3
    stddev = np.sqrt(2 / int(inputs.shape[3]))
    stddev = np.sqrt(2 / (1 * 1 * int(inputs.shape[3])))
    inputs = tf.layers.conv2d(inputs, filters=filters, kernel_size=1,padding='same', 
                             kernel_initializer=tf.random_normal_initializer(stddev=stddev),
                             kernel_regularizer=tf.contrib.layers.l2_regularizer(1.0),
                             name=name + '_3',data_format='channels_first')
    inputs = batch_norm(inputs, training, name + '_3')
