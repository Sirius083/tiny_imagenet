...
VAL_STEP = int(ceil(VAL_DATA_NUM/BATCH_SIZE))     # 遍历一次validaion需要多少个step
...
with g.as_default():
  with tf.device(':/cpu:0'):
      ...
      test_iterator = val_data.make_one_shot_iterator()
  ...
  with tf.Session() as sess:
      ...
      print(next_element)
      # train_next(tuple): float32 (64,56,56,3), unit8 (64) 
      train_next = sess.run(next_element, feed_dict={handle: train_iterator_handle})
      # print('train_next', train_next)
'''
...
with tf.Session() as sess:
     train_iterator_handle = sess.run(train_iterator.string_handle())
     # 问题: iamges_eval 都是零
     images_eval, labels_eval = sess.run([images, labels], feed_dict = {handle: train_iterator_handle})
